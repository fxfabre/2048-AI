...
from src.AI.ai_base import BaseAi
from src.GameGrids.GameGridLight import GameGridLight
...
class ai_TDlearning(BaseAi):
    ...
    def ChooseMove(self, grid, current_state, epsilon):
        score_for_move = {"left": 0, "right": 0, "up": 0, "down": 0}
    ...
    def ChooseMove_old(self, grid, current_state, epsilon):
        ...
        for i in range(4):
            if moves[i].canMove:
                scoreMax = max(scoreMax, moves[i].score)
