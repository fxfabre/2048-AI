...
class BaseGrid2048:
    ...
    def get_symetric_states_for_grid(self):
        current_state = self.GetState()
        equivalent_state = {"nb_transfo": 0, "transfos": [], "state": current_state}
        ...
        for tx in ["symetry_axis_x", None]:
            for ty in ["symetry_axis_y", None]:
                for rotate in ["rotate_90", "rotate_180", "rotate_270", None]:
                    ...
                    state = loc_grid.GetState()
                    if state < equivalent_state["state"] or (
                        (state == equivalent_state["state"]) and (len(transfos) < equivalent_state["nb_transfo"])
                    ):
                        equivalent_state = {"nb_transfo": len(transfos), "transfos": transfos, "state": state}
    ...
    def add_tile(self, row, col, tile_to_add):
        if self.can_add_tile(row, col):
            ...
            print(f"Unable to add tile at ({row}, {col})")

    def set_tile(self, row, col, value):
        if self.matrix[row, col] != 0:
            raise Exception(f"Tile not empty at ({row}, {col}). can't add tile {value}")
