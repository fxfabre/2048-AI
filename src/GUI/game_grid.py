...
class GameGrid(TK.Canvas):
    ...
    def register_tile(self, tile_id, tile_object, raise_error=False):
        ...
        if tile_id not in self.tiles:
            ...
            raise KeyError(f"tile id '{tile_id}' is already registered.")
    ...
    def grid_size(self):
        ...
        return ((self.grid_width - self.half_high), (self.grid_height - self.half_high))
...
class GridMatrix:
    ...
    def add(self, object_, row, column, raise_error=False):
        ...
        if self.matrix[row][column] is None:
            ...
            return True
        if raise_error:
            raise GridError(f"cannot add object at (row, column) = ({row}, {column}): busy location.")
        # failed
        return False
    ...
    def get_object_at(self, row, column, raise_error=False):
        ...
        if raise_error and _object is None:
            raise GridError(f"no object found at (row, column) = ({row}, {column}): empty location.")
        return _object
    ...
    def reset_matrix(self):
        ...
        self.__matrix = [[None] * self.columns for i in range(self.rows)]
