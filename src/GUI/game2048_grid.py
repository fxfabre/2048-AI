...
class Game2048Grid(GG.GameGrid):
    ...
    def animate_text_try_again(self, item_id, value):
        ...
        if value == "#ffffff":
            _btn = ttk.Button(self, text="Play", command=self.owner.new_game)
    ...
    def game_over(self, tk_event=None, *args, **kw):
        ...
        _anim_text1.start_after(
            delay=800,
            interval=50,
            sequence=("#c9bdb4", "#d0c5be", "#d7cdc8", "#ded5d2", "#e5dddc", "#ece5e6", "#f3edf0", "#ffffff"),
        )
        ...
        if self.is_full():
            ...
            for _row in range(self.rows):
                ...
                for _column in range(self.columns):
                    ...
                    if _tile1 and (
                        (_tile2 and _tile1.value == _tile2.value) or (_tile3 and _tile1.value == _tile3.value)
                    ):
                        # the show must go on!
                        return False
